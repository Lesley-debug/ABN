...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals($_SESSION['user_login_csrf'] ?? '', $csrfToken)) {
        ...
        if ($stmtLock === false) {
            error_log('Login attempt lock check prepare failed: ' . $conn->error);
            ...
            $stmtLock->execute();
            $lockCounts = $stmtLock->get_result();

            if ($lockCounts === false) {
                error_log('Login attempt lock check get_result failed: ' . $stmtLock->error);
                $message = 'Unable to complete login right now. Please try again later.';
            } else {
                $lockCounts = $lockCounts->fetch_assoc();
                ...
                if ((int) ($lockCounts['ip_hits'] ?? 0) >= $maxAttempts || (int) ($lockCounts['id_hits'] ?? 0) >= $maxAttempts) {
                    ...
                    if ($stmt === false) {
                        error_log('Login SELECT prepare failed: ' . $conn->error);
                        $message = 'Unable to complete login right now. Please try again later.';
                    } else {
                        ...
                        if ($result !== false && $result->num_rows === 1) {
                            ...
                            error_log('Login SELECT get_result failed: ' . $stmt->error);
                            $message = 'Unable to complete login right now. Please try again later.';
                        }
