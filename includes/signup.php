...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!hash_equals($_SESSION['user_signup_csrf'] ?? '', $token)) {
        ...
        if ($username === '' || $email === '' || $password === '') {
            ...
            if ($check === false) {
                error_log('Signup SELECT prepare failed: ' . $conn->error);
                ...
                if ($result === false) {
                    error_log('Signup SELECT get_result failed: ' . $check->error);
                    $message = 'Unable to complete signup right now. Please try again later.';
                } else {
                    ...
                    if ($exists) {
                        ...
                        if ($stmt === false) {
                            ...
                            error_log('Signup INSERT execute failed: ' . $stmt->error);
