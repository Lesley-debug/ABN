...
function forgotRateLimitExceeded(mysqli $conn, string $ipAddress, int $maxAttempts = 5, int $windowMinutes = 15): bool
{
    ...
    if ($countStmt === false) {
        error_log('Forgot password rate limit count prepare failed: ' . $conn->error);
        return true;
    }
    ...
    if ($countResult === false) {
        error_log('Forgot password rate limit count get_result failed: ' . $countStmt->error);
        return true;
    }
    $hits = (int) (($countResult->fetch_assoc()['hits'] ?? 0));

    $insertStmt = $conn->prepare('INSERT INTO request_limits (action_key, ip_address) VALUES (?, ?)');
    if ($insertStmt === false) {
        error_log('Forgot password rate limit insert prepare failed: ' . $conn->error);
    } else {
    ...
    if ($cleanupStmt === false) {
        error_log('Forgot password rate limit cleanup prepare failed: ' . $conn->error);
    } else {
    ...
    if (!hash_equals($_SESSION['user_forgot_csrf'] ?? '', $token)) {
        ...
        if (forgotRateLimitExceeded($conn, $ipAddress)) {
            ...
            if (!filter_var($email, FILTER_VALIDATE_EMAIL)) {
                ...
                if ($stmt === false) {
                    error_log('Forgot password SELECT prepare failed: ' . $conn->error);
                } else {
                    ...
                    if ($result !== false && $result->num_rows === 1) {
                        ...
                        error_log('Forgot password SELECT get_result failed: ' . $stmt->error);
                    }
                }
            }
